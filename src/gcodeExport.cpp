...
void GCodeExport::writeExtrusion(Point3 p, double speed, double extrusion_mm3_per_mm, PrintFeatureType feature, bool update_extrusion_offset, coord_t drum_radius)
{
    ...
    writeExtrusion(p.x, p.y, p.z, speed, extrusion_mm3_per_mm, feature, update_extrusion_offset, drum_radius);
}
...
void GCodeExport::writeExtrusion(int x, int y, int z, double speed, double extrusion_mm3_per_mm, PrintFeatureType feature, bool update_extrusion_offset, coord_t drum_radius)
{
    ...
    assert(currentPosition != no_point3);
    assert(Point3(x, y, z) != no_point3);
    //sorry utheta get pretty big sometimes
    //assert((Point3(x,y,z) - currentPosition).vSize() < MM2INT(400)); // no crazy positions (this code should not be compiled for release)
    ...
    if (diff.vSizeMMCyl(drum_radius) > 0) {
    ...
    double new_e_value = current_e_value + extrusion_per_mm * diff.vSizeMMCyl(drum_radius + z);
